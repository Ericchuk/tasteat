...
export default function DishSelect(){
    const options = ['cheaper', 'Luxury', 'Fierce']

    // const optionValues = options.map((item) => {
    //     <option value={item.option} key={item.id}>{item.option}</option>
    // })
    ...
    return(
        <aside className='mt-5 flex justify-between items-center'>
            ...
            <select id="dropdown" className='bg-sidebar border border-line text-white rounded-lg w-44 h-12 flex justify-center items-center px-10 py-1.5 appearance-none' onClick={changeFilter}>
