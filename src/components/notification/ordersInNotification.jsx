...
export default function OrderInNotification() {
  const { notificationOrder,removeCart, notificationFilter } = useContext(AppContext);

  const list = notificationOrder.map((item) => {
    const {status, out, id,menu,customerName,customerImage,totalPayment,totalPurchasePaid} = item
    return (
      <ul key={id}>
